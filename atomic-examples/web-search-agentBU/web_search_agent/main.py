...
from web_search_agent.config import get_base_url, get_api_key, get_fast_llm, get_searxng_url, get_searxng_max_results
...
search_tool = SearxNGSearchTool(config=SearxNGSearchToolConfig(base_url=get_searxng_url(), max_results=get_searxng_max_results()))

# Initialize the BaseAgent
agent = BaseAgent(
    config=BaseAgentConfig(
        client=instructor.from_openai(openai.OpenAI(base_url=get_base_url(), api_key=get_api_key()), mode=instructor.Mode),
        model=get_fast_llm(),
        memory=memory,
    )
)
